...
仪表盘采用**暖低饱和色系**，以 Tailwind Stone 为基底（`#f5f0ed` 背景），Amber 为功能强调色。所有平台色均从品牌原色降饱和 30-40% 并加入暖调，使图表与页面氛围协调统一。

月度趋势折线去除数据点标记，仅保留平滑曲线；散点图右下角以暖琥珀渐变暗示性价比最优方向。完整配色思路及色值对照详见 **[design.md](design.md)**。
